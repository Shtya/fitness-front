...
return <section id={id} className={`py-14 sm:py-20 ${className}`}>{children}</section>;
}
...
return (
<div className={`relative rounded-2xl border border-slate-200 bg-white/80 backdrop-blur shadow-[0_8px_30px_rgba(0,0,0,0.05)] ${className}`}>
{children}
</div>
);
}
...
{Icon && <div className="mt-1 w-10 h-10 rounded-xl bg-indigo-50 text-indigo-700 grid place-items-center"><Icon className="w-5 h-5"/></div>}
<div>
