...
export default function AddToHomeGuide({ storageKey = 'a2hs_guide_dismissed_v1', autoShowDelayMs = 1200 }) {
  ...
  const isSafari = !/chrome|crios|fxios|edg/i.test(navigator.userAgent) && (/safari/i.test(navigator.userAgent) || /iphone|ipad|ipod/i.test(navigator.userAgent));
  ...
  const autoKey = useMemo(() => {
    ...
    return 'generic';
  }, [isAndroid, isSamsung, isChrome, isEdge, isFirefox, isIOS, isSafari, isIOSAltBrowser]);
  ...
  return (
    <div dir='rtl' className='fixed inset-0 z-[60] flex  items-center justify-center'>
      <div className='absolute inset-0 bg-slate-900/40 backdrop-blur-sm' onClick={closeGuide} />
      <div className='relative w-[92%] sm:max-w-lg rounded-3xl border border-indigo-200/60 shadow-2xl overflow-hidden'>
        ...
        <div className='bg-white/80 backdrop-blur-2xl px-5 sm:px-6 pt-1 pb-3'>
          {isAndroid && deferredPrompt && (
            <button onClick={triggerInstall} disabled={installing} className='w-full rounded-2xl border border-indigo-200 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-medium py-2.5'>
              {installing ? t('installing') : t('installButton')}
            </button>
          )}

          <h4 className='font-semibold text-slate-800 mt-5 mb-3'>{t('stepsTitle')}</h4>
          <ol className='space-y-2'>
            {steps.map((line, i) => (
              <li key={i} className='flex gap-3 items-start'>
                <span className='mt-0.5 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-indigo-600 text-white text-xs font-semibold'>{i + 1}</span>
          ...
          <div className='mt-5 flex flex-col sm:flex-row items-stretch sm:items-center gap-3'>
            <CheckBox label={t('dontShowAgain')} initialChecked={dontShowAgain} onChange={checked => setDontShowAgain(checked)} className='mt-3' />

            <button onClick={closeGuide} className='sm:ml-auto rounded-xl px-4 py-2.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 transition'>
              {t('close')}
            </button>
          </div>
        </div>
