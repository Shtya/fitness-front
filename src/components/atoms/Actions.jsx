...
const SIZE_MAP = {
  ...
  md: { btn: "w-8 h-8",   icon: 14, labelText: "text-[11px]", radius: "rounded-xl"   },
  lg: { btn: "w-10 h-10", icon: 17, labelText: "text-xs",     radius: "rounded-xl"   },
};
  ...
  return createPortal(
    <AnimatePresence>
      {open && (
        ...
        >
          <div className="rounded-2xl bg-white border border-slate-200/80 overflow-hidden py-1.5"
    ...
    <div className={cn(
      ...
      {hasMore && (
        <div className="relative" ref={moreBtnRef}>
          <Tooltip label="More actions" color={VARIANTS.slate.tooltip}>
