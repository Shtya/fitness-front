...
export default function Select({cnLabel , options = [], value = null, onChange = () => {}, placeholder, searchable = true, disabled = false, clearable = true, className = '', label, cnInputParent, allowCustom = false, createHint = 'Write a new category…' }) {
  ...
  const updatePosition = useCallback(
    preferredPlacement => {
      ...
      const leftRaw = triggerRect.left;
      const clampedLeft = Math.max(8, Math.min(leftRaw, viewportWidth - width - 8));

      let placement = preferredPlacement || coords.placement || 'bottom';
      ...
      if (menuHeight) {
        ...
        if (placement === 'bottom') {
          top = Math.min(triggerRect.bottom + gap, viewportHeight - menuHeight - gap);
        } else {
          top = Math.max(triggerRect.top - menuHeight - gap, gap);
        }
      } else {
      ...
      setCoords(prev => {
        ...
        if (prev.top === next.top && prev.left === next.left && prev.width === next.width && prev.placement === next.placement) {
          return prev;
        }
        return next;
      });
