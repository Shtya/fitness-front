...
export default function Select({
  ...
  return (
    <div ref={rootRef} className={`relative ${className}`}>
      ...
      <button type='button' ref={buttonRef} onClick={() => (open ? closeMenu() : openMenu())} onKeyDown={onKeyDown} disabled={disabled} className={['h-[43px] group relative w-full inline-flex items-center justify-between', 'rounded-xl border bg-white px-3.5 py-2.5 text-sm', disabled ? 'cursor-not-allowed opacity-60' : 'cursor-pointer', 'transition-colors', 'border-slate-300 hover:border-slate-400 focus:border-indigo-500', 'focus:outline-none focus:ring-4 focus:ring-indigo-100'].join(' ')} aria-haspopup='listbox' aria-expanded={open}>
        <span className={`truncate text-left ${selected ? 'text-slate-900' : 'text-gray-500'}`}>{selected ? selected.label : placeholder || 'Select…'}</span>

        <span className='ml-3 flex items-center gap-1'>
          {clearable && selected && !disabled && <X className='h-4 w-4 opacity-60 hover:opacity-100 transition' onClick={clear} />}
      ...
      {portalReady &&
        ...
        createPortal(
          <div role='listbox' aria-activedescendant={activeIndex >= 0 ? `opt-${activeIndex}` : undefined} className='z-[99999999] fixed mt-0' style={{ top: coords.top, left: coords.left, width: coords.width }}>
            <div ref={listRef} className='max-h-72 overflow-auto rounded-2xl border border-slate-200 bg-white shadow-lg ring-1 ring-black/5' onKeyDown={onKeyDown}>
              {/* Search row */}
              {searchable && (
                ...
                {filtered.map((item, idx) => {
                  ...
                  return (
                    <li
                      ...
                      className={['mx-1 my-0.5 rounded-xl px-3 py-2 text-sm flex items-center justify-between select-none cursor-pointer', isActive ? 'bg-indigo-50' : 'bg-transparent', isSelected ? 'text-indigo-700' : 'text-slate-700', 'hover:bg-indigo-50'].join(' ')}
