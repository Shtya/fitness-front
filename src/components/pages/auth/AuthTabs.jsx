...
export default function AuthTabs({setView}) {
  ...
  return (
    <LayoutGroup id='auth-tabs'>
      <div role='tablist' aria-label='Authentication' className='mb-10 grid grid-cols-3 gap-2 rounded-lg bg-gray-100/80 pb-[3px] p-1 text-sm font-medium ring-1 ring-black/5 shadow-sm backdrop-blur'>
        {TABS.map(t => {
          ...
          return (
            <motion.button
              ...
              whileHover={{ y: -1 }}
              whileTap={{ scale: 0.98 }}
              className=' cursor-pointer relative select-none rounded-lg px-3 py-2 outline-none transition-colors focus-visible:ring-2 focus-visible:ring-emerald-500/60'>
               {isPreviewed && <motion.span layoutId='active-pill' className='absolute inset-0 rounded-lg bg-gradient-to-r from-emerald-500 to-emerald-400 shadow-lg' transition={{ type: 'spring', stiffness: 350, damping: 30 }} />}
