...
const SearchComponent = memo(function SearchComponent({ searchText, setSearchText, placeholder, className }) {
  ...
  return (
    <div className={`relative flex-1 max-w-[240px] sm:min-w-[260px] ${className}`}>
      ...
      <input value={searchText} onChange={e => setSearchText(e.target.value)} placeholder={placeholder} className={['h-11 w-full px-8 rounded-lg', 'border border-slate-200 bg-white/90 text-slate-900', 'shadow-sm hover:shadow transition', 'focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-200/40'].join(' ')} />
      {!!searchText && (
