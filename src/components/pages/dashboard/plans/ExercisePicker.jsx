...
import { categoryMap } from '@/app/[locale]/dashboard/workouts/page';
...
export const ExercisePicker = memo(function ExercisePicker({ open, onClose, onDone, dayId, initialSelected = [] }) {
	...
	return createPortal(
		<AnimatePresence>
			{open ? (
				<motion.div key='exercise-picker' className='fixed inset-0 z-[1000000000]' initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
					...
					>
						...
						<div className='sticky top-0 z-10 border-b border-white/10 text-white'>
							<div className='absolute z-[-1] inset-0 overflow-hidden'>
								...
								<div
									className='absolute -bottom-16 -right-8 h-60 w-60 rounded-full blur-3xl opacity-30'
							...
							<div className='mx-auto max-w-7xl  px-4 sm:px-6'>
								<div className='flex items-center justify-between py-4'>
									...
									<SearchComponent searchText={searchText} setSearchText={setSearchText} placeholder={t('picker.searchPlaceholder')} className='max-sm:hidden' />
								</div>
							</div>
							...
							<div className=' '>
								<div className='mx-auto flex items-center justify-between gap-2 flex-wrap max-w-7xl px-4 sm:px-6 py-3'>
									<SearchComponent searchText={searchText} setSearchText={setSearchText} placeholder={t('picker.searchPlaceholder')} className=' sm:hidden' />

									{/* Tabs */}
									{tabs.length > 1 && (
										<div className='overflow-x-auto '>
											{/* ✅ subtle themed bg */}
											<TabsPill hiddenArrow id='picker-cats' className='!bg-white/15 !border !border-white/20' tabs={tabs} active={activeCat} onChange={setActiveCat} />
										</div>
									)}
								</div>
							</div>
											...
											return (
												<motion.button
													...
													className={[
														...
														checked
															? 'border-[color:var(--color-primary-400)] ring-2 ring-[color:var(--color-primary-200)] bg-[color:var(--color-primary-50)] shadow-md'
						...
						<div className='sticky bottom-0 z-10 border-t border-slate-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60'>
							<div className='mx-auto max-w-7xl px-4 sm:px-6'>
								<div className='flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between'>
									...
									<div className='order-1 rtl:order-[-1] flex items-center justify-end gap-2 sm:order-2'>
										<button
											type='button'
											onClick={onClose}
											className='inline-flex items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-800 shadow-sm hover:bg-slate-50 active:scale-[.98] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-slate-400/30 transition'
										>
											{t('actions.close')}
										</button>
										...
										<button
											...
											className={[
												...
												'text-white active:scale-[.98] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[color:var(--color-primary-200)]/40',
												!selectedCount ? 'opacity-50 cursor-not-allowed' : 'hover:opacity-95',
											].join(' ')}
											style={{
							...
							{showVideo && (
								<motion.div className='fixed inset-0 z-[1100] flex items-center justify-center p-4' initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
									...
									<motion.div
										...
										className='relative w-full max-w-3xl rounded-lg bg-black overflow-hidden shadow-2xl'
									>
										<button
											onClick={() => setShowVideo(null)}
											className='absolute right-2 top-2 z-10 inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20 active:scale-95'
