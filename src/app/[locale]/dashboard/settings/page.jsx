/* 
  ...
  - reminders id as string
  - controlled report checkboxes + textarea
  - OG image clear/replace
*/
...
import {
  Info, Save, Link2, Loader2, Upload as UploadIcon, Download,
  Radio, Palette, BellRing, Globe, Sparkles, Check as CheckIcon,
...
const CardShell = ({ title, desc, right, open, onToggle, children }) => (
  <div className="rounded-lg border border-slate-200 bg-white shadow-inner ring ring-slate-100 overflow-hidden">
    <button
      onClick={onToggle}
      className="w-full grid grid-cols-[1fr_auto] items-center gap-3 p-4 border-b border-slate-100"
    >
...
const Button = ({ children, color = 'primary', className = '', ...props }) => {
  ...
  return (
    <button
      {...props}
      className={[
        'inline-flex items-center gap-2 rounded-lg px-3.5 py-2 text-sm font-semibold shadow-sm transition active:scale-[.98]',
        styles[color] || styles.primary, className,
      ].join(' ')}
    >{children}</button>
  );
};
...
const Chip = ({ children }) => (
  <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 text-slate-700 px-2 py-0.5 text-xs border border-slate-200">{children}</span>
);
...
function UploadBox({ file, onFile, previewUrl, onClear, label }) {
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className="rounded-lg border-2 border-dashed border-slate-300 hover:border-indigo-300 transition bg-slate-50/60 p-4"
    >
      {!file ? (
        ...
        <div className="flex items-center gap-3">
          ...
          <Button color="danger" onClick={onClear}><X className="w-4 h-4" />Remove</Button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="relative">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full rounded-lg border border-slate-200 p-3 text-left hover:bg-slate-50 transition"
      >
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            className="absolute z-10 mt-2 w-56 rounded-lg border border-slate-200 bg-white p-3 shadow-lg"
          >
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="h-9 w-14 bg-transparent border border-slate-200 rounded-md"
              />
              <Input value={custom} onChange={(v) => setCustom(v)} className="flex-1" placeholder="#000000" />
              <Button color="neutral" onClick={() => choose(custom)}>Apply</Button>
            </div>
          </motion.div>
        ) : null}
      </AnimatePresence>
...
function uiToApi({ org, siteMeta, loader, dhikrEnabled, dhikrItems, activeDhikrId, theme, report, reminders }) {
  return {
    ...
    dhikrItems: (dhikrItems || []).map(d => ({ id: d.id, text: d.text })),
    ...
    reminders: (reminders || []).map(r => ({ id: String(r.id), title: r.title, time: r.time })),
  };
}
...
export default function SettingsPage() {
  ...
  const sections = { org: 'org', site: 'site', loader: 'loader', dhikr: 'dhikr', theme: 'theme', reports: 'reports', reminders: 'reminders' };
  ...
  const onOgFile = async (file) => {
    setOgFile(file);
    if (!file) { setOgPreviewUrl(''); return; }
  ...
  const [dhikrItems, setDhikrItems] = useState([
    { id: 1, text: 'سُبْحَانَ اللَّهِ' }, { id: 2, text: 'الْحَمْدُ لِلَّهِ' }, { id: 3, text: 'اللَّهُ أَكْبَرُ' },
  ]);
  ...
  const weekdayOptions = useMemo(() => (
    ['Saturday','Sunday','Monday','Tuesday','Wednesday','Thursday','Friday'].map(d => ({ id: d, label: d }))
  ), []);
  const [report, setReport] = useState({
    enabled: true,
    day: 'Sunday',
    time: '09:00',
    items: { weightTrend: true, mealAdherence: true, workoutCompletion: true, waterIntake: true, checkinNotes: true, nextFocus: true, latestPhotos: false },
    customMessage: '',
  });
  ...
  const buildTemplateFromItems = useCallback((items) => {
  ...
  const toggleReportItem = useCallback((key, checked) => {
    setReport((s) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const { data } = await api.get('/settings'); // lang added by interceptor
        if (!alive) return;
        ...
        const custom = u.report.customMessage?.trim()
          ? u.report.customMessage
          : buildTemplateFromItems(u.report.items);
  ...
  const saveAll = async () => {
    setSaving(true);
    try {
      const dto = uiToApi({ org, siteMeta, loader, dhikrEnabled, dhikrItems, activeDhikrId, theme, report, reminders });
  ...
  return (
    <div className="mx-auto max-w-7xl px-3 sm:px-6 py-6 space-y-6 pb-28">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <input ref={importInputRef} type="file" accept="application/json" className="hidden" onChange={onImportChange} />
          <Button color="neutral" onClick={onExport}><Download className="w-4 h-4" />{t('export')}</Button>
          <Button color="neutral" onClick={onImportClick}><UploadIcon className="w-4 h-4" />{t('import')}</Button>
        </div>
      </div>
      ...
      <CardShell
        ...
        right={<Chip><Radio className="w-3.5 h-3.5" /> {org.timezone}</Chip>}
      >
        ...
        <Row label={t('org.defaultLang.label')} hint={t('org.defaultLang.hint')}>
          <div className="flex items-center gap-2">
            <Button type="button" color={org.defaultLang === 'ar' ? 'primary' : 'neutral'} onClick={() => setOrg((s) => ({ ...s, defaultLang: 'ar' }))}>AR</Button>
            <Button type="button" color={org.defaultLang === 'en' ? 'primary' : 'neutral'} onClick={() => setOrg((s) => ({ ...s, defaultLang: 'en' }))}>EN</Button>
          </div>
        </Row>
        ...
        <Row label={t('org.homeSlug.label')} hint={t('org.homeSlug.hint')}>
          <div className="flex gap-2">
            ...
            <Button color="neutral"><Link2 className="w-4 h-4" />{t('org.homeSlug.preview')}</Button>
          </div>
        </Row>
      </CardShell>
      ...
      <CardShell
        title={t('site.title')}
        desc={t('site.desc')}
        open={isOpen(sections.site)}
        onToggle={() => toggleOpen(sections.site)}
        right={<Globe className="w-4 h-4 text-slate-500" />}
      >
          ...
          <div className="space-y-2">
            {(siteMeta.ogImageUrl || ogPreviewUrl) && (
              <div className="flex items-center gap-3">
                <div className="w-28 h-20 overflow-hidden rounded-lg border border-slate-200 bg-white">
                  <img
                    src={ogPreviewUrl || siteMeta.ogImageUrl}
                    className="w-full h-full object-cover"
                    alt="og-preview"
                  />
                </div>
                <Button color="danger" onClick={clearOg}><Trash2 className="w-4 h-4" />{t('remove') || 'Remove'}</Button>
              </div>
            )}
            <UploadBox
              file={ogFile}
      ...
      <CardShell
        ...
        desc={t('loader.desc')}
        open={isOpen(sections.loader)}
        onToggle={() => toggleOpen(sections.loader)}
      >
        ...
        <Row label={t('loader.message')} hint={t('loader.messageHint')}>
          <div className="flex gap-2">
            ...
            <Button color="neutral" onClick={() => setLoaderPreviewOpen(true)}>{t('loader.previewBtn')}</Button>
          </div>
        </Row>
      ...
      <CardShell
        title={t('dhikr.title')}
        desc={t('dhikr.desc')}
        open={isOpen(sections.dhikr)}
        onToggle={() => toggleOpen(sections.dhikr)}
        right={<Sparkles className="w-4 h-4 text-amber-500" />}
      >
          ...
          <div className="space-y-2">
            {dhikrItems.map((item) => (
              <label key={item.id} className="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2">
                ...
                <Button color="ghost" onClick={() => setDhikrItems((list) => list.filter((x) => x.id !== item.id))}>✕</Button>
              </label>
            ))}
            <Button color="neutral" onClick={() => setDhikrItems((list) => [...list, { id: Date.now(), text: '' }])}>+ {t('add')}</Button>
          </div>
        </Row>
      </CardShell>
      ...
      <CardShell
        ...
        open={isOpen(sections.theme)}
        onToggle={() => toggleOpen(sections.theme)}
        right={<Palette className="w-4 h-4 text-slate-500" />}
      >
        ...
        <Row label={t('branding.previewLabel')} hint={t('branding.previewHint')}>
          <div className="space-y-3">
            <div
              ...
              style={{ background: `linear-gradient(135deg, ${theme.palette.primary}, ${theme.palette.secondary})` }}
            >
              <div className="p-5 md:p-7 text-white">
                ...
                <div
                  ...
                  style={{ background: theme.palette.surface, color: theme.palette.onSurface, border: '1px solid rgba(15,23,42,0.08)' }}
                >
            ...
            <div
              ...
              style={{ background: theme.palette.background, color: theme.palette.onBackground }}
            >
              <div
                className="rounded-lg px-3 py-2 inline-flex items-center gap-2"
                style={{ background: theme.palette.primary, color: theme.palette.onPrimary }}
              >
      ...
      {/* Reports & Automations */}
      <CardShell
        title={t('reportsAuto.title')}
        desc={t('reportsAuto.desc')}
        open={isOpen(sections.reports)}
        onToggle={() => toggleOpen(sections.reports)}
        right={<BellRing className="w-4 h-4 text-slate-500" />}
      >
            ...
            {Object.entries(report.items).map(([key, val]) => (
              <label key={key} className="flex items-center gap-2 rounded-md border border-slate-200 p-2">
                <input
                  ...
                  className="h-4 w-4"
                  checked={!!val}
                  onChange={(e) => toggleReportItem(key, e.target.checked)}
                />
        ...
        <Row label={t('reportsAuto.template.label')} hint={t('reportsAuto.template.hint')}>
          <Textarea
            ref={templateRef}
            value={report.customMessage}
            onChange={(e) => setReport((s) => ({ ...s, customMessage: e.target.value }))}
          />
        </Row>
      </CardShell>
      ...
      <CardShell
        ...
        open={isOpen(sections.reminders)}
        onToggle={() => toggleOpen(sections.reminders)}
      >
          ...
          <div className="space-y-2">
            {reminders.map((r) => (
              <div key={r.id} className="grid sm:grid-cols-[1fr_160px_36px] gap-2 rounded-lg border border-slate-200 p-2">
                ...
                <Button color="ghost" onClick={() => removeReminder(r.id)}>✕</Button>
              </div>
            ))}
          </div>
        </Row>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/70">
        <div className="mx-auto max-w-7xl px-3 sm:px-6 py-3 flex items-center justify-end gap-2">
          <Button color="neutral" onClick={() => window.location.reload()}>{t('cancel')}</Button>
        ...
        <div className="relative h-[70vh]">
          ...
          <div className="absolute left-1/2 bottom-6 -translate-x-1/2 bg-white/80 backdrop-blur px-4 py-2 rounded-lg border border-slate-200 shadow">
            <div className="text-slate-800 text-sm font-medium">
              {loader.message || t('loader.messagePh')}
            </div>
...
function AddReminderForm({ onAdd }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <Button color="neutral" onClick={() => { onAdd({ title, time }); setTitle(''); }}>
        + {t('add')}
      </Button>
    </div>
  );
