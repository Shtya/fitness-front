                                ...
                                >
                                  ...
                                  >
                                    <div className="flex items-center gap-2.5">
                                      ...
                                      <ChevronRight
                                        className={`h-4 w-4 flex-shrink-0 transition-all rtl:scale-x-[-1] ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-50'}`}
                                        style={{ color: 'var(--color-primary-500)' }}
                                      />
                                    </div>
                                  </motion.button>

                                  {/* Quick actions row (hover / active) */}
                                  <div
                                    className={`flex items-center gap-1 px-3 pb-2.5 transition-all ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                                  >
                                    <IconBtn tooltip={t('actions.copy_link')} onClick={e => { e.stopPropagation(); copyLink(form.id); }} size="sm">
                                      <LinkIcon className="h-3 w-3" />
                                    </IconBtn>
                                    <IconBtn tooltip={t('actions.duplicate')} onClick={e => { e.stopPropagation(); handleDuplicateForm(form); }} size="sm">
                                      <Files className="h-3 w-3" />
                                    </IconBtn>
                                    {canEdit(form) && (
                                      <>
                                        <IconBtn tooltip={t('actions.edit')} onClick={e => { e.stopPropagation(); openEditFormModal(form, true); }} variant="primary" size="sm">
                                          <PencilLine className="h-3 w-3" />
                                        </IconBtn>
                                        <IconBtn tooltip={t('actions.delete')} onClick={e => { e.stopPropagation(); setDeletingId(form.id); setShowDeleteModal(true); }} variant="danger" size="sm">
