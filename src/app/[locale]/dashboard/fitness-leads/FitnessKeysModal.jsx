...
import { Check, ExternalLink, KeyRound, LoaderCircle, Trash2, X } from 'lucide-react';
import { fitnessLeadsApi } from './fitness-leads-api';

export default function FitnessKeysModal({ open, onClose, isAr, copy, onSaved }) {
	...
	const load = async () => {
		...
		setError('');
		try {
			const res = await fitnessLeadsApi.credentials();
			setData(res);
			setActiveId(current => current || res?.catalog?.[0]?.id || null);
		} catch (err) {
	...
	const onSave = async () => {
		...
		setSuccess('');
		try {
			const status = await fitnessLeadsApi.saveCredential(active.id, fields);
	...
	const onRemove = async () => {
		...
		setRemoving(true);
		try {
			const status = await fitnessLeadsApi.removeCredential(active.id);
							...
							{catalog.map(item => {
								...
								return (
									<button
										...
										className={`mb-1 flex w-full flex-col rounded-xl px-3 py-2.5 text-start ${
											active?.id === item.id ? 'bg-white shadow-sm ring-1 ring-emerald-200' : 'hover:bg-white/80'
										}`}
									>
										...
										<span className={`mt-1 text-[11px] font-medium ${on ? 'text-emerald-700' : 'text-slate-400'}`}>
											{on
						...
						<div className="overflow-y-auto p-5">
							{active && (
								<>
									...
									<div className="my-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-950">
										...
										<div className="mt-3 flex flex-wrap gap-2">
											<a
												...
												target="_blank"
												rel="noreferrer"
												className="inline-flex items-center gap-1.5 rounded-lg border border-amber-300 bg-white px-3 py-1.5 text-xs font-bold"
											>
									...
									{(active.fields || []).map(field => (
										<div key={field.key} className="mb-3">
											...
											<input
												...
												className="mt-1 h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm outline-none focus:border-emerald-400 focus:bg-white"
											/>
										</div>
									))}
									...
									<div className="mt-2 flex flex-wrap gap-2">
										...
										{activeStatus?.source === 'database' && (
											<button
												...
												disabled={removing}
												onClick={onRemove}
												className="inline-flex h-10 items-center gap-2 rounded-xl border border-rose-200 px-4 text-sm font-semibold text-rose-700"
											>
												{removing ? (
													<LoaderCircle className="h-4 w-4 animate-spin" />
												) : (
