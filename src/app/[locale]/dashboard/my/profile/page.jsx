...
function HeaderStatPill({ label, value, icon: Icon, delay = 0 }) {
	return (
		...
		>
			...
			<p className="!text-xs sm:text-2xl font-black text-white leading-none tabular-nums">
				{value ?? 0}
			</p>
		</motion.div>
	);
				...
				{[
					...
					>
						...
						<div className="flex items-baseline gap-1">
							<span className={`text-lg sm:text-xl font-black tabular-nums leading-none ${s.color}`}>
								{s.val}
							</span>
				...
				{sorted.map((m, i) => {
					...
					return (
						...
						>
							...
							<span className={`text-sm font-black leading-tight mt-0.5 ${isActive ? 'text-[var(--color-primary-700)]' : 'text-slate-800'}`}>
								{m.weight ?? '-'}
							</span>
							...
							<div className="flex-1 min-w-0">
								...
								<div className="flex items-baseline gap-1 flex-wrap">
									<motion.span
										...
										className="text-[22px] sm:text-3xl font-black text-slate-900 leading-none tabular-nums"
									>
										{goal.value || '--'}
									</motion.span>
	...
	return (
		<div className="min-h-screen w-[calc(100%+14px)] rtl:mr-[-7px] ltr:ml-[-7px] mt-[-7px]">
				...
				<div className="space-y-3">
					{['lighting', 'distance', 'angles', 'cameraHeight', 'timer', 'clothes', 'background', 'frequency'].map(key => (
						<div key={key} className="flex gap-3 p-3.5 rounded-lg bg-gradient-to-r from-[var(--color-primary-50)] to-[var(--color-secondary-50)] border border-[var(--color-primary-100)]">
							...
							<p className="text-sm text-slate-700 leading-relaxed">{t(`tips.${key}`)}</p>
						</div>
					))}
				</div>
			</Modal>
			...
			<Modal open={!!confirmDeletePhotoId} onClose={() => setConfirmDeletePhotoId(null)} title={t('modals.confirmDeleteTitle')} maxW="max-w-md">
				<p className="text-sm text-slate-600 mb-6 leading-relaxed">{t('messages.deletePhotoConfirm')}</p>
			...
			<Modal open={!!confirmDeleteMeasurementId} onClose={() => setConfirmDeleteMeasurementId(null)} title={t('modals.confirmDeleteTitle')} maxW="max-w-md">
				<p className="text-sm text-slate-600 mb-6 leading-relaxed">{t('messages.deleteMeasurementConfirm')}</p>
				<div className="flex gap-3">
