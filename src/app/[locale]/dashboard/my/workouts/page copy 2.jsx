...
async function fetchAllDaySets(userId, exerciseNames, date) {
  ...
  const entries = await Promise.all(
    exerciseNames.map(async name => [name, await fetchDaySets(userId, name, date)])
  );
...
async function fetchExerciseStats(userId, name, windowDays = 90) {
  const [seriesRes, topRes, histRes] = await Promise.all([
    api.get('/prs/stats/e1rm-series', { params: { userId, exerciseName: name, bucket: 'week', windowDays } }),
    api.get('/prs/stats/top-sets', { params: { userId, exerciseName: name, top: 5 } }),
    api.get('/prs/history', { params: { userId, exerciseName: name } }),
  ]);
  return {
...
function RestTimerCard({ initialSeconds, audioEl, className = '' }) {
  ...
  useEffect(() => {
    if (!running && duration > 0 && remaining === 0) {
      ...
      try {
        ...
        el.play();
        const t = setTimeout(() => {
          try { el.pause(); el.currentTime = 0; el.loop = false; } catch {}
        }, 30000);
  ...
  const toggleInput = () => { setShowInput(v => !v); setTimeout(() => inputRef.current?.focus(), 0); };
  const commitInput = e => { setSeconds(mmssToSeconds(e.target.value)); setShowInput(false); };
        ...
        {!running ? (
          <>
            ...
            <div className='inline-flex rounded-lg overflow-hidden border border-slate-200'>
              <button onClick={dec} className='px-2 py-2 text-xs hover:bg-slate-50' title='-15s'><Minus size={14} /></button>
              <button onClick={inc} className='px-2 py-2 text-xs hover:bg-slate-50 border-l border-slate-200' title='+15s'><Plus size={14} /></button>
              <button onClick={toggleInput} className='px-2 py-2 text-xs hover:bg-slate-50 border-l border-slate-200' title='Edit time'><Edit3 size={14} /></button>
            </div>
            {showInput && (
              <div className='relative'>
                <input
                  ref={inputRef}
                  type='text'
                  defaultValue={toMMSS(seconds)}
                  onBlur={commitInput}
                  onKeyDown={e => e.key === 'Enter' && commitInput(e)}
                  className='h-9 w-24 rounded-lg border border-slate-200 bg-white px-3 text-slate-900 shadow-inner outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition tabular-nums'
                  placeholder='mm:ss'
                />
...
/* =================== RADIO / PODCAST PANEL =================== */
function AudioHub({ open, onClose, podcasts = [] }) {
  const audioRef = useRef(null);
  const [mode, setMode] = useState('quran'); // 'quran' | 'podcast'
  const [playing, setPlaying] = useState(false);
  const [current, setCurrent] = useState(podcasts[0]?.url || '');

  const QURAN_STREAM = 'https://qurango.net/radio/mix'; // إذاعة القرآن الكريم (بديل عام)

  useEffect(() => { if (!open) try { audioRef.current?.pause(); setPlaying(false); } catch {} }, [open]);
  useEffect(() => { setCurrent(podcasts[0]?.url || ''); }, [podcasts]);

  const src = mode === 'quran' ? QURAN_STREAM : current;

  const toggle = async () => {
    const el = audioRef.current;
    if (!el) return;
    if (playing) { try { el.pause(); setPlaying(false); } catch {} }
  ...
  return (
    <AnimatePresence>
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className='fixed inset-0 z-[70] bg-black/40 backdrop-blur-sm'>
          <motion.div initial={{ y: 24 }} animate={{ y: 0 }} exit={{ y: 24 }} className='absolute bottom-0 left-0 right-0 md:left-auto md:right-6 md:bottom-6 md:w-[460px] rounded-2xl bg-white shadow-xl border border-slate-200'>
            <div className='p-3 flex items-center justify-between border-b border-slate-100'>
              <div className='flex items-center gap-2 font-semibold'><Headphones size={18} /> استماع: قرآن / بودكاست</div>
              <button onClick={onClose} className='p-2 rounded-lg hover:bg-slate-100'><X size={16} /></button>
            </div>

            <div className='p-3 space-y-3'>
              <div className='flex items-center gap-2'>
                <button onClick={() => setMode('quran')} className={`px-3 py-1.5 rounded-lg text-sm border ${mode==='quran'?'border-indigo-300 bg-indigo-50 text-indigo-700':'border-slate-200 hover:bg-slate-50'}`}>
                  <Radio size={14} className='inline mr-1'/> إذاعة القرآن الكريم
                </button>
                <button onClick={() => setMode('podcast')} className={`px-3 py-1.5 rounded-lg text-sm border ${mode==='podcast'?'border-indigo-300 bg-indigo-50 text-indigo-700':'border-slate-200 hover:bg-slate-50'}`}>بودكاست</button>
              </div>

              {mode === 'podcast' && (
                <div className='max-h-48 overflow-y-auto border border-slate-200 rounded-xl'>
                  {podcasts.map((p, i) => (
                    <button key={i} onClick={() => setCurrent(p.url)} className='w-full text-left px-3 py-2 border-b last:border-b-0 hover:bg-slate-50'>
                      <div className='font-medium text-sm truncate'>{p.title}</div>
                      <div className='text-xs text-slate-500 truncate'>{p.url}</div>
                    </button>
                  ))}
                  {!podcasts.length && <div className='p-3 text-sm text-slate-500'>No podcasts yet. Add some URLs.</div>}
                </div>
              )}

              <div className='flex items-center justify-between rounded-xl border border-slate-200 p-3'>
                <div className='text-sm'>{mode === 'quran' ? 'إذاعة القرآن الكريم' : (podcasts.find(p=>p.url===current)?.title || 'Podcast')}</div>
                <button onClick={toggle} className='inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm border-slate-200 hover:bg-slate-50'>
                  {playing ? <><Pause size={14}/> إيقاف</> : <><Play size={14}/> تشغيل</>}
                </button>
              </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className='fixed inset-0 z-[80] bg-black/30'>
          <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }} className='mx-auto mt-24 w-[92%] max-w-md rounded-2xl border border-slate-200 bg-white shadow-xl'>
            <div className='p-3 border-b border-slate-100 flex items-center justify-between'>
              <div className='font-semibold flex items-center gap-2'><SettingsIcon size={18}/> Settings</div>
              <button onClick={onClose} className='p-2 rounded-lg hover:bg-slate-100'><X size={16}/></button>
            </div>
            <div className='p-4 space-y-3'>
              ...
              <div className='flex items-center justify-end gap-2 pt-2'>
                <button onClick={onClose} className='px-3 py-1.5 rounded-lg border border-slate-200 text-sm hover:bg-slate-50'>Cancel</button>
                <button onClick={() => { onChange(sel); onClose(); }} className='px-3 py-1.5 rounded-lg border border-indigo-200 bg-indigo-50 text-indigo-700 text-sm hover:bg-indigo-100'>Save</button>
              </div>
            </div>
          </motion.div>
        </motion.div>
...
export default function MyWorkoutsPage() {
  ...
  function buildDailyPRPayload(exName) {
    const records = (workout?.sets || [])
      ...
      .map(s => ({ id: s.serverId, weight: Number(s.weight) || 0, reps: Number(s.reps) || 0, done: !!s.done, setNumber: Number(s.set) || 1 }));
  ...
  async function saveSingleSet(setObj) {
    const ex = workout?.exercises.find(e => e.id === currentExId);
    if (!ex) return;
    const payload = { exerciseName: ex.name, date: todayISO(), set: { id: setObj.serverId, weight: Number(setObj.weight) || 0, reps: Number(setObj.reps) || 0, done: !!setObj.done, setNumber: Number(setObj.set) || 1 } };
    try {
      const data = await upsertAttempt(USER_ID, payload.exerciseName, payload.date, payload.set);
      const serverRecords = data?.records || [];
      setWorkout(w => ({
        ...w,
        sets: w.sets.map(s => (s.id === setObj.id ? (serverRecords.find(r => Number(r.setNumber) === Number(setObj.set)) ? { ...s, serverId: serverRecords.find(r => Number(r.setNumber) === Number(setObj.set)).id } : s) : s)),
      }));
      await Promise.all([loadOverview(30), loadExerciseStats(ex.name, 90)]);
      // mirror
      ...
      // still keep unsaved flag off only if no diff left
      markUnsaved(workout, setUnsaved);
    } catch (err) {
  ...
  const dayTabs = weekOrder
    ...
    .map(d => ({ key: d, label: d, name: (plan?.program?.days || []).find(x => x.id === d)?.name || weeklyProgram[d].name }));
  ...
  return (
    <div className='space-y-5 sm:space-y-6'>
      ...
      <div className='flex items-center justify-between flex-wrap gap-2 sm:gap-3'>
        ...
        <div className='flex items-center gap-2'>
          <button onClick={() => setAudioOpen(true)} className='inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-3 py-1.5 text-sm hover:bg-slate-50'><Headphones size={16}/> Listen</button>
          <button onClick={() => setSettingsOpen(true)} className='inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-3 py-1.5 text-sm hover:bg-slate-50'><SettingsIcon size={16}/> Settings</button>
          <TabsPill id='my-workouts-tabs' tabs={[{ key: 'workout', label: 'Workout', icon: Dumbbell }, { key: 'history', label: 'History', icon: HistoryIcon }]} active={tab} onChange={k => setTab(k)} />
        </div>
      </div>
      ...
      {tab === 'workout' && (
        <div className='rounded-2xl border border-slate-200 bg-white p-2.5 sm:p-3'>
          <div className='flex items-center justify-between gap-2'>
            ...
            <div className='flex items-center gap-2'>
              <button onClick={() => setDrawerOpen(true)} className='md:hidden inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-3 py-2 text-sm hover:bg-slate-50'><MenuIcon size={16}/> Exercises</button>
              <button
                onClick={() => setSessionStart(prev => (prev ? null : Date.now()))}
                className={`inline-flex items-center gap-2 rounded-2xl px-3 py-2 text-sm border transition ${sessionStart ? 'border-green-200 bg-green-50 text-green-700' : 'border-indigo-200 bg-indigo-50 text-indigo-700'}`}
                title='Start/Stop workout timer'
              >
                <Clock size={16}/> {sessionStart ? `Running • ${formatElapsed(elapsed)}` : 'Start Workout'}
              </button>
            </div>
          </div>
        </div>
                  ...
                  <div className='flex flex-col lg:flex-row gap-4'>
                      ...
                      <div className='rounded-xl border border-slate-200 bg-white overflow-hidden'>
                        {!hasExercises && (
                          <div className='p-6'>
                            <div className='rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center'>
                              <div className='mx-auto mb-3 w-12 h-12 rounded-full bg-white shadow grid place-items-center'><Dumbbell size={18} className='text-slate-500' /></div>
                        ...
                        {hasExercises && (
                          <>
                            ...
                            <div className='w-full bg-black relative rounded-[10px_10px_0_0] overflow-hidden' style={{ height: 400 }}>
                              {currentExercise && (activeMedia === 'video' || activeMedia === 'video2') && currentExercise[activeMedia] ? (
                                ...
                                <img key={currentExercise?.id + '-image'} src={currentExercise?.img} alt={currentExercise?.name || 'Exercise'} className='w-full h-full object-cover' />
                              )}
                              <div className='absolute bottom-2 right-2 flex items-center gap-2 bg-black/40 backdrop-blur px-2 py-1 rounded-lg'>
                                <button onClick={() => setActiveMedia('image')} className={`text-xs px-2 py-1 rounded ${activeMedia === 'image' ? 'bg-white text-slate-900' : 'text-white hover:bg-white/10'}`} title='Show image'><ImageIcon size={14} /></button>
                                <button onClick={() => setActiveMedia('video')} className={`text-xs px-2 py-1 rounded ${activeMedia === 'video' ? 'bg-white text-slate-900' : 'text-white hover:bg-white/10'}`} title='Show video' disabled={!currentExercise?.video}><VideoIcon size={14} /></button>
                                {currentExercise?.video2 && (
                                  <button onClick={() => setActiveMedia('video2')} className={`text-xs px-2 py-1 rounded ${activeMedia === 'video2' ? 'bg-white text-slate-900' : 'text-white hover:bg-white/10'}`} title='Show video 2' disabled={!currentExercise?.video2}><VideoIcon size={14} /></button>
                                )}
                              </div>
                            </div>
                            ...
                            <div className='lg:mx-2 mb-4 border border-slate-200 rounded-lg overflow-hidden'>
                                      ...
                                      <tr key={s.id} className={`hover:bg-indigo-50/40 transition-colors ${i % 2 === 1 ? 'bg-slate-50/30' : 'bg-white'}`}>
                                        ...
                                        <td className='py-2.5 px-3'>
                                          <input
                                            type='string'
                                            value={s.weight}
                                            onChange={e => setWorkout(w => ({ ...w, sets: w.sets.map(x => (x.id === s.id ? { ...x, weight: +e.target.value } : x)) }))}
                                        ...
                                        <td className='py-2.5 px-3'>
                                          <input
                                            type='string'
                                            value={s.reps}
                                            onChange={e => setWorkout(w => ({ ...w, sets: w.sets.map(x => (x.id === s.id ? { ...x, reps: +e.target.value } : x)) }))}
                                        ...
                                        <td className='py-2.5 px-3'>
                                          <CheckBox initialChecked={s.done} onChange={() => { toggleDone(s.id); setUnsaved(true); persistLocalBuffer({ ...workout }); }} />
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
                              ...
                              <div className='flex items-center justify-between px-3 py-2 text-[11px] text-slate-500 bg-slate-50/60'>
                                <div className='flex items-center gap-2'>
                                  <button onClick={removeSetFromCurrentExercise} className='inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs text-slate-700 hover:bg-slate-50 disabled:opacity-50' disabled={currentSets.length <= 1}><Minus size={14} /></button>
                                  <button onClick={addSetForCurrentExercise} className='inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs text-slate-700 hover:bg-slate-50'><Plus size={14} /></button>
                                </div>
                                <div className='flex items-center gap-2'>
                                  {unsaved && <span className='text-xs text-amber-600'>Unsaved changes buffered locally</span>}
                                  <button onClick={saveDayToServer} className='inline-flex items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 py-1.5 text-xs text-indigo-700 hover:bg-indigo-100' title='Save/Upsert this exercise day'><SaveIcon size={14}/> Save</button>
                                </div>
                              </div>
                            </div>
                          </>
                    ...
                    <div className='hidden lg:block w-80'>
                      <ExerciseList
                        ...
                        onPick={(ex) => { setCurrentExId(ex.id); setExercisePick(ex.name); setActiveMedia('image'); }}
                      />
                    </div>
                  </div>
                )}
              ...
              <div className='overflow-x-auto'>
                <table className='w-full text-sm rounded-xl border border-slate-200'>
                  ...
                  <tbody className='divide-y divide-slate-100'>
                    ...
                    {!attempts?.length && (
                      <tr>
                        <td colSpan={6} className='py-3 px-3 text-sm text-slate-500'>No attempts yet.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <div className='mt-3'>
              <DataTable
                columns={[
                  ...
                  { header: 'Sets', accessor: 'setsDone', cell: r => (<span className='tabular-nums'>{Number(r?.setsDone || 0)}/{Number(r?.setsTotal || 0)}</span>) },
                ]}
      ...
      {/* Audio hub (Qur’an / Podcast) */}
      <AudioHub open={audioOpen} onClose={() => setAudioOpen(false)} podcasts={podcasts} />
      ...
      <SettingsPopup
        ...
        onChange={(s) => {
          setAlertSound(s);
          try { localStorage.setItem(LOCAL_KEY_SETTINGS, JSON.stringify({ alertSound: s })); } catch {}
        }}
      />
    </div>
  );
          ...
          >
            <div className='flex items-center gap-3'>
              <div className='w-10 h-10 overflow-hidden rounded-lg bg-slate-100 grid place-items-center'>
                {ex.img ? <img src={ex.img} alt='' className='object-cover w-full h-full' /> : <Dumbbell size={16} className='text-slate-500' />}
              </div>
              <div className='min-w-0 flex-1'>
                <div className='font-medium truncate'>{idx + 1}. {ex.name}</div>
...
function markUnsaved(workout, setUnsaved) {
  try { persistLocalBuffer(workout); } catch {}
  setUnsaved(true);
}
function persistLocalBuffer(workout) {
  ...
  (workout.exercises || []).forEach(ex => {
    buf[ex.name] = (workout.sets || [])
      .filter(s => s.exName === ex.name)
      .map(s => ({ set: s.set, weight: s.weight, reps: s.reps, done: !!s.done }));
  });
