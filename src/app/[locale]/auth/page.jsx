...
const LoginForm = ({ onLoggedIn }) => {
  ...
  const onSubmit = async data => {
    ...
    try {
      ...
      // IMPORTANT: store translation KEYS, not translated strings
    ...
    <motion.form initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} onSubmit={handleSubmit(onSubmit)} className='w-full space-y-4' noValidate>
      ...
      <div className='space-y-1.5'>
        ...
        <div className='relative'>
          <input type={showPassword ? 'text' : 'password'} placeholder={t('enterPassword')} autoComplete='current-password' className='w-full h-11 rounded-lg border border-gray-200 bg-white px-3.5 rtl:pl-11 ltr:pr-11 outline-none ring-0 focus:border-gray-300 focus:ring-4 focus:ring-gray-100 transition' {...register('password')} />
          <button type='button' aria-label={showPassword ? t('a11y.hidePassword') : t('a11y.showPassword')} onClick={() => setShowPassword(s => !s)} className='absolute inset-y-0 rtl:left-2.5 ltr:right-2.5 my-auto grid place-items-center rounded-lg p-2 hover:bg-gray-50 active:scale-95 transition'>
...
function HeroSide() {
  const t = useTranslations('auth');
  const perks = [t('hero.perkPersonalized'), t('hero.perkProgress'), t('hero.perkChat'), t('hero.perkPrivacy')];
